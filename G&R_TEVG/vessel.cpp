...
vessel::vessel() { //Default constructor
    ...
    P_h = 0, f_h = 0, bar_tauw_h = 0, Q_h = 0, P_prev=0;
    sigma_h = { 0 };

    //Current loading quantities
    lambda_th_curr = 0, lambda_z_curr = 0;
    P = 0, f = 0, bar_tauw = 0, Q = 0;
    sigma = { 0 }, Cbar = { 0 }, lambda_alpha_tau = { 0 }, lambda_z_tau = { 0 };
...
void vessel::initializeTEVG(string scaffold_name, string immune_name, vessel const &native_vessel, double n_days_inp, double dt_inp) {
    ...
    double fd_p_1, fd_p_2, fd_p, ps_p, epsilon_p, epsilon_seal;
    ...
    //For sheep grafts, pore size determined from sealant volume fraction, assumed first constituent
    epsilon_seal = epsilon_p1_0;
    ps_p = -sqrt(M_PI) / 4 * (1 + M_PI / (2 * log(1 - epsilon_seal))) * fd_p;
    ...
    double chronic_infl = (ps_norm / ps_p + fd_p / fd_norm - 2) > 0;

    double K_i_p_trans = K_i_p_mic * (ps_p / ps_norm * fd_p / fd_norm) + K_i_p_wound;
    double K_i_p_steady = K_i_p_mic * (ps_norm / ps_p + fd_p / fd_norm - 2) * chronic_infl;
    double K_i_d = K_i_d_max;
    double Ki_rhop = K_i_p_trans + K_i_p_steady; //K_i_p_mic; //
    double Ki_drhopdt = Ki_rhop; //K_i_p_wound; //

    //Polymer degradation initialization
    double Q_p1, Q_p2, Q_gnd;
    double dQdt_p1, dQdt_p2;
    
    double ups_rhoRp, ups_drhoRpdt;
    double ups_d_rhoRp, ups_d_drhoRpdt;
    double s_gnd_off = 0.0, epsilonR_gnd_min = 0.90, s = 0.0;
    double window_fun = 0.0;
    double test = 0.0;

    double max_deg_rate;
    dQdt_p1 = abs(dQpdt(zeta_p1 * gamma_p_d2, k_p1 * gamma_p_d1, zeta_p1 * gamma_p_d2));
    dQdt_p2 = abs(dQpdt(zeta_p2 * gamma_p_d2, k_p2 * gamma_p_d1, zeta_p2 * gamma_p_d2));
    if (dQdt_p1 * epsilonR_alpha_0[0] > dQdt_p2 * epsilonR_alpha_0[1]){
        dQdt_p2 = dQpdt(zeta_p1 * gamma_p_d2, k_p2 * gamma_p_d1, zeta_p2 * gamma_p_d2);
    }
    else{
        dQdt_p1 = dQpdt(zeta_p2 * gamma_p_d2, k_p1 * gamma_p_d1, zeta_p1 * gamma_p_d2);
    }
    max_deg_rate = (dQdt_p1 * epsilonR_alpha_0[0] + dQdt_p2 * epsilonR_alpha_0[1]) / (epsilonR_alpha_0[0] + epsilonR_alpha_0[1]);

    //Window for controlling duration of inflammatory period
    double window_end;
    Immune_in >> window_end;

    //Ratio of col to SMC inflammatory response
    double rat_smc2col_p, rat_smc2col_d;
    Immune_in >> rat_smc2col_p >> rat_smc2col_d;
    ...
    for (int sn = 1; sn < nts; sn++) {
        ...
        //dQdt_p1 = -k_p1 * gamma_p_d1 * exp(k_p1 * gamma_p_d1 * (s - zeta_p1 * gamma_p_d2)) /
                //(1 + exp(k_p1 * gamma_p_d1 * (s - zeta_p1 * gamma_p_d2))) * Q_p1;
        //dQdt_p2 = -k_p2 * gamma_p_d1 * exp(k_p2 * gamma_p_d1 * (s - zeta_p2 * gamma_p_d2)) /
                //(1 + exp(k_p2 * gamma_p_d1 * (s - zeta_p2 * gamma_p_d2))) * Q_p2;
        ...
        Q_p1 = Qp(s, k_p1 * gamma_p_d1, zeta_p1 * gamma_p_d2);
        Q_p2 = Qp(s, k_p2 * gamma_p_d1, zeta_p2 * gamma_p_d2);
        Q_gnd = (s > s_gnd_off)* ((1 - epsilonR_gnd_min) * exp(-k_gnd_h * (s - s_gnd_off)) + epsilonR_gnd_min)
            + (s <= s_gnd_off) * 1;

        dQdt_p1 = dQpdt(s, k_p1 * gamma_p_d1, zeta_p1 * gamma_p_d2);
        dQdt_p2 = dQpdt(s, k_p2 * gamma_p_d1, zeta_p2 * gamma_p_d2);
        ...
        if (s < window_end) {
                window_fun = 0.5 * (cos(3.1415 / (window_end) * s) + 1);
            }
        else {
                window_fun = 0;
            }

        ups_rhoRp = Ki_rhop * window_fun * (1 - exp(-delta_i_p * s)) *
            (Q_p1 * epsilonR_alpha_0[0] + Q_p2 * epsilonR_alpha_0[1]) / (epsilonR_alpha_0[0] + epsilonR_alpha_0[1]);

        ups_drhoRpdt = - Ki_drhopdt * window_fun * (1 - exp(-delta_i_p * s)) / max_deg_rate *
                (dQdt_p1 * epsilonR_alpha_0[0] + dQdt_p2 * epsilonR_alpha_0[1]) / (epsilonR_alpha_0[0] + epsilonR_alpha_0[1]);

        for (int alpha = 0; alpha < n_alpha; alpha++){
            //Check if inflammatory constituent
            if (alpha_infl[alpha] == 1) {
                //Check if SMC at index 7
                if (alpha == 7){
                    ups_infl_p[nts * alpha + sn] = rat_smc2col_p * (ups_rhoRp + ups_drhoRpdt);
                    ups_infl_d[nts * alpha + sn] = rat_smc2col_d * K_i_d;
                }
                else{
                    ups_infl_p[nts * alpha + sn] = ups_rhoRp + ups_drhoRpdt;
                    ups_infl_d[nts * alpha + sn] = K_i_d;
                }
...
void vessel::printTEVGOutputs() {
    GnR_out << a[sn] << "\t" << h[sn] << "\t" << rhoR[sn] << "\t" << rhoR_alpha[0 * nts + sn] << "\t"
        << rhoR_alpha[1 * nts + sn] << "\t" << rhoR_alpha[7 * nts + sn] << "\t"
        << rhoR_alpha[8 * nts + sn] + rhoR_alpha[9 * nts + sn] + rhoR_alpha[10 * nts + sn] + rhoR_alpha[11 * nts + sn] << "\t"
        << bar_tauw << "\t" << bar_tauw_h << "\t" << P << "\t" << P_h << "\t" << f << "\t" << f_h
        << "\t" << Q << "\t" << Q_h << "\n";
    GnR_out.flush();

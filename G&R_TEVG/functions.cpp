...
int find_iv_geom(void* curr_vessel) {
    ...
    a_mid_high = 1.10 * ((struct vessel*) curr_vessel)->a_mid[sn];
    ...
    do {
        ...
        status = gsl_root_test_interval(a_mid_low, a_mid_high, 0, pow(10, -16));
...
double Qp(double s, double k, double zeta){
    double Qpval = (1 + exp(-k * zeta)) / (1 + exp(k * (s - zeta)));
    return Qpval;
}

double dQpdt(double s, double k, double zeta){
    double Qpval = Qp(s, k, zeta);
    double dQpdtval = -k * exp(k * (s - zeta)) /
                (1 + exp(k * (s - zeta))) * Qpval;
    return dQpdtval;
}